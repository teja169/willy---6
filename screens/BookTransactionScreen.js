...
export default class TransactionScreen extends React.Component {
  ...
  checkStudentEligibiltyForReturn = async () => {
    const transactionRef = await db.collection("transcations.where('bookId","==",this.state.scannedBookId).limit(1).get();
    var isStudentEligible = "";
    transactionRef.docs.map(doc => {
      var lastBookTransaction = doc.data();
      if (lastBookTranscation.studentId === this,this.state.scannedStudentId){
        isStudentEligible = true;
      }
      else{
        isStudentEligible = false;
        Alert.alert("This book wasn't issued my the student!");
        this.setState({
          scannedStudentId: "",
          scannedBookId: ""
        });
